import { execSp } from "../db/execSp.js";
import sql from "mssql";

export async function listModelos(req, res) {
  try {
    const result = await execSp("sp_modelos", [
      { name: "accion", type: sql.VarChar(20), value: "LIST" },
    ]);

    res.json({ ok: true, data: result.recordset });
  } catch (err) {
    console.error(err);
    res.status(500).json({ ok: false, msg: "Error listando modelos" });
  }
}

export async function listModelosCombo(req, res) {
  try {
    const result = await execSp("sp_modelos", [
      { name: "accion", type: sql.VarChar(20), value: "COMBO" },
    ]);

    res.json({ ok: true, data: result.recordset });
  } catch (err) {
    console.error(err);
    res.status(500).json({ ok: false, msg: "Error listando modelos" });
  }
}

export async function getModelo(req, res) {
  try {
    const id = Number(req.params.id);

    const result = await execSp("sp_modelos", [
      { name: "accion", type: sql.VarChar(20), value: "GET" },
      { name: "id", type: sql.Int, value: id },
    ]);

    const row = result.recordset?.[0];

    if (!row) {
      return res.status(404).json({ ok: false, msg: "Modelo no encontrado" });
    }

    res.json({ ok: true, data: row });
  } catch (err) {
    console.error(err);
    res.status(500).json({ ok: false, msg: "Error consultando modelo" });
  }
}

export async function createModelo(req, res) {
  try {
    const m = req.body;

    const result = await execSp("sp_modelos", [
      { name: "accion", type: sql.VarChar(20), value: "INSERT" },
      { name: "id_marca", type: sql.TinyInt, value: Number(m.id_marca) },
      { name: "id_submarca", type: sql.TinyInt, value: Number(m.id_submarca) },
      { name: "activo", type: sql.Bit, value: m.activo ?? true },
    ]);

    res.json({ ok: true, id: result.recordset?.[0]?.id });
  } catch (err) {
    console.error(err);
    res.status(500).json({
      ok: false,
      msg: err?.message || "Error insertando modelo",
    });
  }
}

export async function updateModelo(req, res) {
  try {
    const id = Number(req.params.id);
    const m = req.body;

    const result = await execSp("sp_modelos", [
      { name: "accion", type: sql.VarChar(20), value: "UPDATE" },
      { name: "id", type: sql.Int, value: id },
      { name: "id_marca", type: sql.TinyInt, value: Number(m.id_marca) },
      { name: "id_submarca", type: sql.TinyInt, value: Number(m.id_submarca) },
      { name: "activo", type: sql.Bit, value: m.activo ?? true },
    ]);

    res.json({
      ok: true,
      id: result.recordset?.[0]?.id ?? id,
      activo: m.activo ?? true,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({
      ok: false,
      msg: err?.message || "Error actualizando modelo",
    });
  }
}

export async function setActivoModelo(req, res) {
  try {
    const id = Number(req.params.id);
    const { activo } = req.body;

    const result = await execSp("sp_modelos", [
      { name: "accion", type: sql.VarChar(20), value: "SET_ACTIVO" },
      { name: "id", type: sql.Int, value: id },
      { name: "activo", type: sql.Bit, value: activo },
    ]);

    res.json({
      ok: true,
      id: result.recordset?.[0]?.id ?? id,
      activo: result.recordset?.[0]?.activo ?? activo,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({
      ok: false,
      msg: err?.message || "Error actualizando estatus de modelo",
    });
  }
}